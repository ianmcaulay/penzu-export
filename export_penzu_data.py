...
def wait_for_page_load(driver):
    loader = driver.find_element_by_class_name('global-loader__wrap')
...
def get_entries_from_entries_url(driver, entries_url):
    ...
    entries = []
    for entry_element in driver.find_elements_by_class_name('entries-list__item'):
        entry_title_element = singleton(entry_element.find_elements_by_css_selector('.title.item__cell'))
        entry_url_element = singleton(entry_title_element.find_elements_by_tag_name('a'))
        entry_url = entry_url_element.get_attribute('href')
        created_at_element = singleton(entry_element.find_elements_by_css_selector('.date.item__cell'))
...
def get_entry_data(driver, entry):
    ...
    text_element = singleton(driver.find_elements_by_class_name('cke_inner'))
    text = text_element.text
    title_element = singleton(driver.find_elements_by_class_name('h1'))
